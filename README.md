...
当前版本定位为最小可用产品（MVP）：优先解决「本地上传富途年度账单 → 按 FIFO 计算已实现盈亏 → 汇率换算 → 导出结果」这条核心链路，帮助用户快速完成基础的资本利得整理。

后续可行演进方向：

1. **扩展为带后端的会员服务**  
   支持账户体系、历史数据留存与跨年度分析，为用户提供长期、连续的盈亏统计、资产回顾和税务辅助能力。

2. **支持更多证券机构与账单类型**  
   在富途年度账单之外，逐步扩展到更多券商、多个证券账户、月结单、日结单等不同来源和周期的交易文件。

3. **支持更多文件格式**  
   从当前 Excel 解析能力扩展到多种 Excel 模板、CSV、PDF 等格式，降低用户整理原始账单的成本。

4. **支持从邮箱自动读取结单**  
   在用户授权的前提下，自动识别并读取邮箱中的券商结单，减少手动下载、上传和整理文件的步骤。

最终期望是从单一的资本利得计算工具，逐步成长为面向个人投资者的证券助手：帮助用户持续理解交易、收益、税务和资产变化。
...
页脚支持通过 `mailto:` 打开用户邮箱客户端发送留言。配置反馈邮箱后，页面会显示「留言反馈」链接：

```bash
VITE_FEEDBACK_EMAIL=your-email@example.com
```

如果未配置 `VITE_FEEDBACK_EMAIL`，页面不会显示留言反馈入口。
